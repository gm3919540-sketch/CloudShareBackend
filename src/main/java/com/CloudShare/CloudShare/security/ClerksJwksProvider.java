...
@Component
public class ClerksJwksProvider {
    @Value("${clerk.jwks-url}")
    private String jwsUrl;
    private final Map<String, PublicKey> keyCahe = new HashMap<>();
    private long lastFetchTime=0;
    private static final long CACHE_TTL =360000; // 1hour

    public PublicKey getPublicKey(String kid) throws Exception{
        if(keyCahe.containsKey(kid) &&System.currentTimeMillis()-lastFetchTime<CACHE_TTL){
            return keyCahe.get(kid);
        }
        refreshKeys();
        return keyCahe.get(kid);
    ...
    private void refreshKeys() throws Exception{
        ...
        JsonNode jwks  = mapper.readTree(new URL(jwsUrl)); //convet this into java class
